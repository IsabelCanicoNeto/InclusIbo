...
class vadSpeech:
    def __init__(self, numdevices, micID, micIDname, start_time, sample_rate=16000, chunk_duration_ms=30, padding_duration_ms=1000,
                 vad_critic=3, filename_wave="test", speakersfile_wave = "nome", silenceDb=40.0, activity = None, condition= "inclusive") :
        ...
        for i in range(numdevices):
            ...
            if micIDname in dev['name']:
                self.device_id = i
                break
        #if self.device_id == 0 : self.device_id = 4
        ...
        self.dbdif = 10.0 # dif from  2 differentes mics db to assume overlap
        ...
        self.idle = True # todo validate
        self.VADminoverlap = 15.0 # number of seconds to consider an overlap
        self.VADminidle = 5.0 # number of seconds to consider idle 3
        self.start_idle = -1
    ...
    def detect_background(self):
        ...
        print("vad_detect_backgound: device {} - micId {} - name: {} - maxInputChannels: {} ".format(self.device_id, self.micID, dev['name'],
                                                                 dev['maxInputChannels']))
    ...
    def _detectThread(self):
        ...
        while not self.leave:
            ...
            while not self.got_a_sentence and not self.leave:
                ...
                #if currdB < 0 : print (f"VAD : erro no microfone {self.micID} device {self.device_id}")
                #if currdB1 < 0 : print (f"VAD : erro no microfone {self.other_mic1ID} device {self.other_mic1.device_id}")
                #if currdB2 < 0 : print (f"VAD : erro no microfone {self.other_mic2ID} device {self.other_mic2.device_id}")

                #if ((currdB1 - currdB) > self.dbdif): active = 0 #
                #if ((currdB2 - currdB) >  self.dbdif): active = 0
                #if currdB < self.silenceDb : active = 0 # todo retirar ou nao VALIDAR
                ...
                if not self.triggered:
                    ...
                    if num_voiced > 0.8 * self.NUM_WINDOW_CHUNKS:
                        ...
                        self.list_speakdB.append((time.time(), "start", 0.0, self.micID , currdB , self.other_mic1ID, currdB1, self.other_mic2ID, currdB2 ))
                        self.time_of_speech.append(time.time())
                        print (f"VAD : start talking mic id :  {self.micID} db {str(currdB)} , other mic ID   {self.other_mic1ID} db {str(currdB1)} , another mic ID   {self.other_mic2ID} db {str(currdB2)}")
                    ...
                    # end point detection
                        ...
                        if (self.idle == False) and (self.start_idle != -1) :
                            ...
                            if duration_idle >= self.VADminidle :
                                ...
                                print (f"VAD : start idle mic id :  {self.micID} db {str(currdB)} other mic ID   {self.other_mic1ID} db {str(currdB1)} "
                                       f"another mic ID   {self.other_mic2ID} db {str(currdB2)} now {time.time()}")
                        ...
                        if self.idle and self.other_mic1.idle and self.other_mic2.idle:
                            ...
                            #print (f"idle sp1 {mic_speakingTime}, sp2 {othermicID1_speakingTime}, sp3 {othermicID2_speakingTime}")
                    ...
                    if num_unvoiced > 0.90 * self.NUM_WINDOW_CHUNKS_END:
                        ...
                        print(f"VAD : stop talking mic id  {self.micID} duration {duration_time} db {str(currdB)} "
                              f"other mic ID   {self.other_mic1ID} db {str(currdB1)} , another mic ID   {self.other_mic2ID} db {str(currdB2)}")
                        self.start_idle = self.endOfLastSpeech
                        self.list_speakdB.append((time.time(), "stop", duration_time, self.micID , currdB , self.other_mic1ID, currdB1, self.other_mic2ID, currdB2 ))
                        # end IN
                    else:
                        # keep talking
                        overlap = False
                        ...
                        # new IN  mark beginning of talking Assumed talking
                        self.idle = False # IN
                        self.start_idle = -1
                        #print (f"talking sp1 {mic_speakingTime}, sp2 {othermicID1_speakingTime}, sp3 {othermicID2_speakingTime}")
                        ...
                        self.list_speakdB.append((time.time(), "talking", duration_time, self.micID , currdB , self.other_mic1ID, currdB1, self.other_mic2ID, currdB2 ))
                        # end IN
    ...
    def printMicMetrics(self):
        ...
        print("microphone mic_id {}".format(self.micID))
    ...
    def logMicMetrics(self):
        ...
        dir = f"logs/{self.condition}/{name}/"
        pathlib.Path(dir).mkdir(parents=True, exist_ok=True)

        file = dir + f"/mic_logs_{self.start_time}.log"
        ...
        logging.info(str(self.micID))
        logging.info("======device ID  ======")
        ...
        logging.info("======Speak dbn ======")
        logging.info(self.list_speakdB)

        mic_logs.append(str(self.micID))
        ...
        mic_logslist.append(self.list_speak_durations)
        mic_logslist.append(self.time_of_speech)
        #mic_logsdb.append(self.list_speakdB)

        return mic_logs, mic_logslist, self.list_speakdB
